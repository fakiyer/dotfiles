...
function getRootDeps(template, context) {
    if (production && template._T_) {
        return template._T_;
    }

    attachDepsAndComponentsToTemplate(template, context);

    var deps = template._T_ = Object.keys(template._U_).map(key => template._U_[key]);
...
function attachDepsAndComponentsToTemplate(target, context) {
    ...
    if (production && target._U_) return;
    ...
    if (meta.tags) {
        meta.tags.forEach(tagPath => {
            ...
            if (tag._U_) Object.assign(deps, tag._U_);
